...
define(["Aras/Client/Controls/Public/TreeGridContainer",
	...
	function (TreeGridContainer, BorderContainer, ContentPane, Graphics, DateTime, Times_Fields, Project, GanttChart, PredecessorType) {
		"use strict";
		...
		var GanttChartContainer = function (contentNode, arasObj) {
			...
			var self = this,
				...
				default_column_titles = "Task,N,Pred,Assigned,Start Date,End Date,Duration,Progress",
				...
				getFontNodeValue = function (currentNode, nodeName) {
					...
					if (fontNode) {
						fontFamily = getNodeValueOrDefault(fontNode, "@family | family", defaultFontFamily);
						tmpStr = getNodeValueOrDefault(fontNode, "@size | size", defaultFontSize.toString());
				...
				loadSubTask = function (wbs, taskNode) {
					...
					//parse "start-date" and "end-date"
						...
						startDate2 = DateTime.Date.DateTimeNull,
						endDate2 = DateTime.Date.DateTimeNull,
					...
					tmpStr = getDateNodeValue(taskNode, "start-date2");
					if (tmpStr) {
						startDate2 = getDateTimeNodeValue(tmpStr);
					}
					tmpStr = getDateNodeValue(taskNode, "end-date2");
					if (tmpStr) {
						endDate2 = getDateTimeNodeValue(tmpStr);
					}
					// parse "duration"
					...
					label = getNodeValueOrDefault(taskNode, "label", "Task " + taskNumber.toString());
					...
					taskProgressColor = getNodeValueOrDefault(taskNode, "progress-color", null);
					//create and initialize Task instance
					task = wbs.createTask(taskId, label, startDate, endDate, startDate2, endDate2, duration);
					task.setIndex(-1);
				...
				loadSubWbs = function (currentNode, parentWbs) {
					...
					for (i = 0; i < currentNode.childNodes.length; i++) {
						...
						if ("wbs" === entityNode.tagName) {
							...
							wbs = (!parentWbs) ? project.createWbs(id, label) : parentWbs.addSubWbs(id, label);
						...
						for (j = 0; j < wbs.getTasks().length; j++) {
							...
							for (k = 0; k < predsList.length; k++) {
								...
								if (res) {
									...
									switch (predType) {
										case "SS":
											task.appendPredecessor(PredecessorType.StartToStart, predTask);
											break;
										case "SF":
											task.appendPredecessor(PredecessorType.StartToFinish, predTask);
											break;
										case "FF":
											task.appendPredecessor(PredecessorType.FinishToFinish, predTask);
											break;
										default:
											task.appendPredecessor(PredecessorType.FinishToStart, predTask);
											break;
									}
								} else {
				...
				initializeContainers = function () {
					self.container = new BorderContainer({ design: "sidebar" }, self.content);
					self.ganttContainer = new ContentPane({ region: "center", style: "overflow:hidden; height:100%;padding:0;", id: "ganttContainer" });
					self.container.addChild(self.ganttContainer);
					self.treeContainer = new ContentPane({ region: "leading", style: "width:30%; height:100%; padding:0;background-color:inherit;", splitter: true, id: "projectTree" });
			...
			this.load = function (xml, node) {
				...
				this.input_date_format = getNodeValueOrDefault(projectNode, "input-date-format", default_input_date_format);
				...
				project.setBackgroundColor(getNodeValueOrDefault(projectNode, "bg-color", Graphics.Colors.White));
				project.setColor(getNodeValueOrDefault(projectNode, "color", Graphics.Colors.DarkGray));
				project.setGridColor(getNodeValueOrDefault(projectNode, "grid-color", Graphics.Colors.DarkGray));
				project.setWeekendColor(getNodeValueOrDefault(projectNode, "week-color", Graphics.Colors.LightGray));
				project.setDependencyColor(getNodeValueOrDefault(projectNode, "dependency-color", Graphics.Colors.Blue));
				project.setBarColor(getNodeValueOrDefault(projectNode, "bar-color", Graphics.Colors.DarkGray));
				project.setFont(getFontNodeValue(projectNode, "font"));
				project.setProgressColor(getNodeValueOrDefault(projectNode, "progress-color", Graphics.Colors.Blue));
				...
				project.setCalendarFormat(getNodeValueOrDefault(projectNode, "display-date-format", default_display_date_format));
				project.setCalendarColor(getNodeValueOrDefault(projectNode, "calendar-color", Graphics.Colors.Black));

				project.setColumnTitles(getNodeValueOrDefault(projectNode, "column-titles", default_column_titles));
